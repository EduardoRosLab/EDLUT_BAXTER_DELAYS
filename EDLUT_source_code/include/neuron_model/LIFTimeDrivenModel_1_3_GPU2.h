...
class LIFTimeDrivenModel_1_3_GPU2 : public TimeDrivenNeuronModel_GPU2 {
	public:
		...
		*/
		 ...
		 *
		 */
		__device__ LIFTimeDrivenModel_1_3_GPU2(float Eexc, float Einh, float Erest, float Vthr, float Cm, float Texc, float Tinh,
		...
		__device__ void EvaluateDifferentialEquation(int index, int SizeStates, float * NeuronState, float * AuxNeuronState, float elapsed_time){
			if(vectorNeuronState_GPU2->LastSpikeTimeGPU[index]>this->tref){
				AuxNeuronState[blockDim.x*blockIdx.x + threadIdx.x] = (NeuronState[SizeStates + index] * (this->eexc - NeuronState[index]) + NeuronState[2 * SizeStates + index] * (this->einh - NeuronState[index]) + grest * (this->erest - NeuronState[index]) + NeuronState[3 * SizeStates + index])*this->inv_cm;
				...
				AuxNeuronState[blockDim.x*blockIdx.x + threadIdx.x] = fraction*(NeuronState[SizeStates + index] * (this->eexc - NeuronState[index]) + NeuronState[2 * SizeStates + index] * (this->einh - NeuronState[index]) + grest * (this->erest - NeuronState[index]) + NeuronState[3 * SizeStates + index])*this->inv_cm;
			}else{
		...
		*/
		__device__ void loadIntegrationMethod_GPU2(char const* integrationName, void ** Buffer_GPU){
			...
			if (cmp4(integrationName, "Euler", 5) == 0){
				this->integrationMethod_GPU2 = (Euler_GPU2<LIFTimeDrivenModel_1_3_GPU2> *) new Euler_GPU2<LIFTimeDrivenModel_1_3_GPU2>(this, Buffer_GPU);
			}
			else if (cmp4(integrationName, "RK2", 3) == 0){
				this->integrationMethod_GPU2 = (RK2_GPU2<LIFTimeDrivenModel_1_3_GPU2> *) new RK2_GPU2<LIFTimeDrivenModel_1_3_GPU2>(this, Buffer_GPU);
			}
			else if (cmp4(integrationName, "RK4", 3) == 0){
				this->integrationMethod_GPU2 = (RK4_GPU2<LIFTimeDrivenModel_1_3_GPU2> *) new RK4_GPU2<LIFTimeDrivenModel_1_3_GPU2>(this, Buffer_GPU);
			}
			else if (cmp4(integrationName, "BDF", 3) == 0 && atoiGPU(integrationName, 3)>0 && atoiGPU(integrationName, 3)<7){
				this->integrationMethod_GPU2 = (BDFn_GPU2<LIFTimeDrivenModel_1_3_GPU2> *) new BDFn_GPU2<LIFTimeDrivenModel_1_3_GPU2>(this, Buffer_GPU, atoiGPU(integrationName, 3));
			}
			else if (cmp4(integrationName, "Bifixed_Euler", 13) == 0){
				this->integrationMethod_GPU2 = (Bifixed_Euler_GPU2<LIFTimeDrivenModel_1_3_GPU2> *) new Bifixed_Euler_GPU2<LIFTimeDrivenModel_1_3_GPU2>(this, Buffer_GPU);
			}
			else if (cmp4(integrationName, "Bifixed_RK2", 11) == 0){
				this->integrationMethod_GPU2 = (Bifixed_RK2_GPU2<LIFTimeDrivenModel_1_3_GPU2> *) new Bifixed_RK2_GPU2<LIFTimeDrivenModel_1_3_GPU2>(this, Buffer_GPU);
			}
			else if (cmp4(integrationName, "Bifixed_RK4", 11) == 0){
				this->integrationMethod_GPU2 = (Bifixed_RK4_GPU2<LIFTimeDrivenModel_1_3_GPU2> *) new Bifixed_RK4_GPU2<LIFTimeDrivenModel_1_3_GPU2>(this, Buffer_GPU);
			}
			else{
