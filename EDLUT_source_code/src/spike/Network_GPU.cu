...
#include "../../include/neuron_model/LIFTimeDrivenModel_1_3_GPU.h"
...
NeuronModel ** Network::LoadNetTypes(string ident_type, string neutype, int & ni, long Currentline, const char * netfile) throw (EDLUTException){
	...
	if (ni<nneutypes && neutypes[ni][0]==0){
		for(int n=0; n<this->GetNumberOfQueues(); n++){
			...
			}else if (ident_type == "LIFTimeDrivenModel_1_4_NEW"){
				neutypes[ni][n] = (LIFTimeDrivenModel_1_4_NEW *) new LIFTimeDrivenModel_1_4_NEW(ident_type, neutype);
			...
			}else if (ident_type == "LIFTimeDrivenModel_1_3_NEW"){
				neutypes[ni][n] = (LIFTimeDrivenModel_1_3_NEW *) new LIFTimeDrivenModel_1_3_NEW(ident_type, neutype);
			}else if (ident_type == "LIFTimeDrivenModel_1_3"){
				neutypes[ni][n] = (LIFTimeDrivenModel_1_3 *) new LIFTimeDrivenModel_1_3(ident_type, neutype);
			}else if (ident_type == "LIFTimeDrivenModel_1_2_NEW"){
				neutypes[ni][n] = (LIFTimeDrivenModel_1_2_NEW *) new LIFTimeDrivenModel_1_2_NEW(ident_type, neutype);
			...
			}else if (ident_type == "Lethargic_LIFTimeDrivenModel_1_1"){
				neutypes[ni][n] = (Lethargic_LIFTimeDrivenModel_1_1 *) new Lethargic_LIFTimeDrivenModel_1_1(ident_type, neutype);
			}else if (ident_type=="HHTimeDrivenModel"){
				neutypes[ni][n] = (HHTimeDrivenModel *) new HHTimeDrivenModel(ident_type, neutype);
			}else if (ident_type=="OriginalHHTimeDrivenModel"){
				neutypes[ni][n] = (OriginalHHTimeDrivenModel *) new OriginalHHTimeDrivenModel(ident_type, neutype);
			}else if (ident_type=="IzhikevichTimeDrivenModel"){
				neutypes[ni][n] = (IzhikevichTimeDrivenModel *) new IzhikevichTimeDrivenModel(ident_type, neutype);
			}else if (ident_type=="AdExTimeDrivenModel"){
				neutypes[ni][n] = (AdExTimeDrivenModel *) new AdExTimeDrivenModel(ident_type, neutype);
			...
			}else if (ident_type=="TimeDrivenPurkinjeCell_IP"){
				neutypes[ni][n] = (TimeDrivenPurkinjeCell_IP *) new TimeDrivenPurkinjeCell_IP(ident_type, neutype);			
			...
			}else if (ident_type=="TimeDrivenInferiorOliveCell"){
				neutypes[ni][n] = (TimeDrivenInferiorOliveCell *) new TimeDrivenInferiorOliveCell(ident_type, neutype);
			...
			}else if (ident_type == "LIFTimeDrivenModel_1_2_GPU_NEW"){
				if (NumberOfGPUs>0){
					neutypes[ni][n] = (LIFTimeDrivenModel_1_2_GPU_NEW *) new LIFTimeDrivenModel_1_2_GPU_NEW(ident_type, neutype);
				}else{
					printf("WARNING: CUDA capable GPU not available. Implementing CPU model intead of GPU model\n");
					neutypes[ni][n] = (LIFTimeDrivenModel_1_2_NEW *) new LIFTimeDrivenModel_1_2_NEW(ident_type, neutype);
				}
			...
			}else if (ident_type=="LIFTimeDrivenModel_1_3_GPU"){
				if(NumberOfGPUs>0){
					neutypes[ni][n] = (LIFTimeDrivenModel_1_3_GPU *) new LIFTimeDrivenModel_1_3_GPU(ident_type, neutype);
				}else{
					printf("WARNING: CUDA capable GPU not available. Implementing CPU model intead of GPU model\n");
					neutypes[ni][n] = (LIFTimeDrivenModel_1_3 *) new LIFTimeDrivenModel_1_3(ident_type, neutype);
				}
			}
			else if (ident_type == "LIFTimeDrivenModel_1_4_GPU_NEW"){
				if (NumberOfGPUs>0){
					neutypes[ni][n] = (LIFTimeDrivenModel_1_4_GPU_NEW *) new LIFTimeDrivenModel_1_4_GPU_NEW(ident_type, neutype);
				}
				else{
					printf("WARNING: CUDA capable GPU not available. Implementing CPU model intead of GPU model\n");
					neutypes[ni][n] = (LIFTimeDrivenModel_1_4_NEW *) new LIFTimeDrivenModel_1_4_NEW(ident_type, neutype);
			...
			}else if (ident_type=="IzhikevichTimeDrivenModel_GPU"){
				if(NumberOfGPUs>0){
					neutypes[ni][n] = (IzhikevichTimeDrivenModel_GPU *) new IzhikevichTimeDrivenModel_GPU(ident_type, neutype);
				}else{
					printf("WARNING: CUDA capable GPU not available. Implementing CPU model intead of GPU model\n");
					neutypes[ni][n] = (IzhikevichTimeDrivenModel *) new IzhikevichTimeDrivenModel(ident_type, neutype);
				}
			}else if (ident_type=="AdExTimeDrivenModel_GPU"){
				if(NumberOfGPUs>0){
					neutypes[ni][n] = (AdExTimeDrivenModel_GPU *) new AdExTimeDrivenModel_GPU(ident_type, neutype);
				}else{
					printf("WARNING: CUDA capable GPU not available. Implementing CPU model intead of GPU model\n");
					neutypes[ni][n] = (AdExTimeDrivenModel *) new AdExTimeDrivenModel(ident_type, neutype);
				}
			}else if (ident_type=="HHTimeDrivenModel_GPU"){
				if(NumberOfGPUs>0){
					neutypes[ni][n] = (HHTimeDrivenModel_GPU *) new HHTimeDrivenModel_GPU(ident_type, neutype);
				}else{
					printf("WARNING: CUDA capable GPU not available. Implementing CPU model intead of GPU model\n");
					neutypes[ni][n] = (HHTimeDrivenModel *) new HHTimeDrivenModel(ident_type, neutype);
				}
        			...
        			if(this->wchanges){
        				for(wcind=0;wcind<this->nwchanges;wcind++){
        					...
        					if(fscanf(fh," %"MAXIDSIZEC"[^ \n]%*[^ ]",ident_type)==1){
        						...
        						}else if (string(ident_type)==string("ExpOptimisedBufferedAdditiveKernel")){
        							this->wchanges[wcind] = new ExpOptimisedBufferedWeightChange(wcind);
        						}	else if (string(ident_type)==string("SinBufferedAdditiveKernel")){
        							this->wchanges[wcind] = new SinBufferedWeightChange(wcind);
								} else if (string(ident_type) == string("SinOptimisedBufferedAdditiveKernel")){
									this->wchanges[wcind] = new SinOptimisedBufferedWeightChange(wcind);
								...
								} else if (string(ident_type) == string("SimetricCosBufferedAdditiveKernel")){
									this->wchanges[wcind] = new SimetricCosBufferedWeightChange(wcind);
