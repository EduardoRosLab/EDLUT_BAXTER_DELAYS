...
SinBufferedWeightChange::SinBufferedWeightChange(int NewLearningRuleIndex):AdditiveKernelChange(NewLearningRuleIndex), exponent(0),
bufferedActivityTimesNoTrigger(0){
}
...
void SinBufferedWeightChange::InitializeConnectionState(unsigned int NumberOfSynapses, unsigned int NumberOfNeurons){
	...
	this->maxTimeMeasured = this->maxpos * 4;
...
void SinBufferedWeightChange::LoadLearningRule(FILE * fh, long & Currentline, string fileName) throw (EDLUTFileException){
