...
#include "../../include/neuron_model/LIFTimeDrivenModel_1_3_GPU2.h"
...
void LIFTimeDrivenModel_1_3_GPU::LoadNeuronModel(string ConfigFile) throw (EDLUTFileException){
	FILE *fh;
	long Currentline = 0L;
...
LIFTimeDrivenModel_1_3_GPU::LIFTimeDrivenModel_1_3_GPU(string NeuronTypeID, string NeuronModelID): TimeDrivenNeuronModel_GPU(NeuronTypeID, NeuronModelID, SecondScale), eexc(0), einh(0), erest(0), vthr(0), cm(0), texc(0), tinh(0),
		tref(0), grest(0){
}

LIFTimeDrivenModel_1_3_GPU::~LIFTimeDrivenModel_1_3_GPU(void){
	DeleteClassGPU2();
}

void LIFTimeDrivenModel_1_3_GPU::LoadNeuronModel() throw (EDLUTFileException){
	this->LoadNeuronModel(this->GetModelID()+".cfg");
}

VectorNeuronState * LIFTimeDrivenModel_1_3_GPU::InitializeState(){
	return this->GetVectorNeuronState();
}


InternalSpike * LIFTimeDrivenModel_1_3_GPU::ProcessInputSpike(Interconnection * inter, double time){
...
__global__ void LIFTimeDrivenModel_1_3_GPU_UpdateState(LIFTimeDrivenModel_1_3_GPU2 ** NeuronModel_GPU2, double CurrentTime){
	(*NeuronModel_GPU2)->UpdateState(CurrentTime);
}

		
bool LIFTimeDrivenModel_1_3_GPU::UpdateState(int index, double CurrentTime){
	VectorNeuronState_GPU *state = (VectorNeuronState_GPU *) State;

	//Update the input current
	for (int i=0; i < state->GetSizeState(); i++){
		state->AuxStateCPU[2 * state->GetSizeState() + i] = this->CurrentSynapses->GetTotalCurrent(i);
//		cout << state->AuxStateCPU[2 * state->GetSizeState() + i] << endl;
	}


	//----------------------------------------------
	if(prop.canMapHostMemory){
		LIFTimeDrivenModel_1_3_GPU_UpdateState<<<N_block,N_thread>>>(NeuronModel_GPU2, CurrentTime);
	}else{
...
enum NeuronModelOutputActivityType LIFTimeDrivenModel_1_3_GPU::GetModelOutputActivityType(){
	return OUTPUT_SPIKE;
}

enum NeuronModelInputActivityType LIFTimeDrivenModel_1_3_GPU::GetModelInputActivityType(){
	return INPUT_SPIKE_AND_CURRENT;
}


ostream & LIFTimeDrivenModel_1_3_GPU::PrintInfo(ostream & out){
	out << "- Leaky Time-Driven Model 1_3: " << this->GetModelID() << endl;
...
void LIFTimeDrivenModel_1_3_GPU::InitializeStates(int N_neurons, int OpenMPQueueIndex){
	...
	float initialization[] = {erest,0.0,0.0,0.0};
...
__global__ void LIFTimeDrivenModel_1_3_GPU_InitializeClassGPU2(LIFTimeDrivenModel_1_3_GPU2 ** NeuronModel_GPU2, 
	...
	if(blockIdx.x==0 && threadIdx.x==0){
		(*NeuronModel_GPU2)=new LIFTimeDrivenModel_1_3_GPU2(eexc, einh, erest, 
			vthr, cm, texc, tinh, tref, grest, integrationName, N_neurons, Buffer_GPU);
	}
}

void LIFTimeDrivenModel_1_3_GPU::InitializeClassGPU2(int N_neurons){
	...
	LIFTimeDrivenModel_1_3_GPU_InitializeClassGPU2<<<1,1>>>(NeuronModel_GPU2,eexc, einh, erest, vthr, 
...
void LIFTimeDrivenModel_1_3_GPU::InitializeVectorNeuronState_GPU2(){
	VectorNeuronState_GPU *state = (VectorNeuronState_GPU *) State;
...
void LIFTimeDrivenModel_1_3_GPU::DeleteClassGPU2(){
    DeleteClass_GPU2<<<1,1>>>(NeuronModel_GPU2);
    cudaFree(NeuronModel_GPU2);
}


bool LIFTimeDrivenModel_1_3_GPU::CheckSynapseType(Interconnection * connection){
