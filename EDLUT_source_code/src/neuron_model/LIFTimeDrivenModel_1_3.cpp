...
#include "../../include/neuron_model/LIFTimeDrivenModel_1_3.h"
...
void LIFTimeDrivenModel_1_3::LoadNeuronModel(string ConfigFile) throw (EDLUTFileException){
	FILE *fh;
	long Currentline = 0L;
	...
	if(fh){
								...
								if(fscanf(fh,"%f",&this->tinh)==1 && this->tinh > 0.0f){
									...
									if (fscanf(fh, "%f", &this->tref) == 1 && this->tref >= 0.0f){
										...
										if (fscanf(fh, "%f", &this->grest) == 1 && this->grest > 0.0f){
											...
											throw EDLUTFileException(TASK_LIF_TIME_DRIVEN_MODEL_1_3_LOAD, ERROR_LIF_TIME_DRIVEN_MODEL_1_2_GREST, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
										}
									} else {
										throw EDLUTFileException(TASK_LIF_TIME_DRIVEN_MODEL_1_3_LOAD, ERROR_LIF_TIME_DRIVEN_MODEL_1_2_TREF, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
									}
								} else {
									throw EDLUTFileException(TASK_LIF_TIME_DRIVEN_MODEL_1_3_LOAD, ERROR_LIF_TIME_DRIVEN_MODEL_1_2_TINH, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
								}
							} else {
								throw EDLUTFileException(TASK_LIF_TIME_DRIVEN_MODEL_1_3_LOAD, ERROR_LIF_TIME_DRIVEN_MODEL_1_2_TEXC, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
							}
						} else {
							throw EDLUTFileException(TASK_LIF_TIME_DRIVEN_MODEL_1_3_LOAD, ERROR_LIF_TIME_DRIVEN_MODEL_1_2_CM, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
						}
					} else {
						throw EDLUTFileException(TASK_LIF_TIME_DRIVEN_MODEL_1_3_LOAD, ERROR_LIF_TIME_DRIVEN_MODEL_1_2_VTHR, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
					}
				} else {
					throw EDLUTFileException(TASK_LIF_TIME_DRIVEN_MODEL_1_3_LOAD, ERROR_LIF_TIME_DRIVEN_MODEL_1_2_EREST, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
				}
			} else {
				throw EDLUTFileException(TASK_LIF_TIME_DRIVEN_MODEL_1_3_LOAD, ERROR_LIF_TIME_DRIVEN_MODEL_1_2_EINH, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
			}
		} else {
			throw EDLUTFileException(TASK_LIF_TIME_DRIVEN_MODEL_1_3_LOAD, ERROR_LIF_TIME_DRIVEN_MODEL_1_2_EEXC, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
		}
		...
		throw EDLUTFileException(TASK_LIF_TIME_DRIVEN_MODEL_1_3_LOAD, ERROR_NEURON_MODEL_OPEN, REPAIR_NEURON_MODEL_NAME, Currentline, ConfigFile.c_str(), true);
	}
	fclose(fh);
}

//this neuron model is implemented in a second scale.
LIFTimeDrivenModel_1_3::LIFTimeDrivenModel_1_3(string NeuronTypeID, string NeuronModelID): TimeDrivenNeuronModel(NeuronTypeID, NeuronModelID, SecondScale), eexc(0), einh(0), erest(0), vthr(0), cm(0), texc(0), tinh(0),
		tref(0), grest(0){
}

LIFTimeDrivenModel_1_3::~LIFTimeDrivenModel_1_3(void)
{
}

void LIFTimeDrivenModel_1_3::LoadNeuronModel() throw (EDLUTFileException){
	this->LoadNeuronModel(this->GetModelID()+".cfg");
}

VectorNeuronState * LIFTimeDrivenModel_1_3::InitializeState(){
	return this->GetVectorNeuronState();
}


InternalSpike * LIFTimeDrivenModel_1_3::ProcessInputSpike(Interconnection * inter, double time){
...
bool LIFTimeDrivenModel_1_3::UpdateState(int index, double CurrentTime){
	//NeuronState[0] --> v 
	//NeuronState[1] --> gexc 
	//NeuronState[2] --> ginh 
...
enum NeuronModelOutputActivityType LIFTimeDrivenModel_1_3::GetModelOutputActivityType(){
	return OUTPUT_SPIKE;
}

enum NeuronModelInputActivityType LIFTimeDrivenModel_1_3::GetModelInputActivityType(){
	return INPUT_SPIKE_AND_CURRENT;
}


ostream & LIFTimeDrivenModel_1_3::PrintInfo(ostream & out){
...
void LIFTimeDrivenModel_1_3::InitializeStates(int N_neurons, int OpenMPQueueIndex){
...
void LIFTimeDrivenModel_1_3::EvaluateSpikeCondition(float previous_V, float * NeuronState, int index, float elapsedTimeInNeuronModelScale){
...
void LIFTimeDrivenModel_1_3::EvaluateDifferentialEquation(float * NeuronState, float * AuxNeuronState, int index, float elapsed_time){
...
void LIFTimeDrivenModel_1_3::EvaluateTimeDependentEquation(float * NeuronState, int index, int elapsed_time_index){
	float limit=1e-9;
...
void LIFTimeDrivenModel_1_3::Calculate_conductance_exp_values(int index, float elapsed_time){
	//excitatory synapse.
...
bool LIFTimeDrivenModel_1_3::CheckSynapseType(Interconnection * connection){
...
void LIFTimeDrivenModel_1_3::loadIntegrationMethod(string fileName, FILE *fh, long * Currentline)throw (EDLUTFileException){
	...
	if (fscanf(fh, "%s", ident_type) == 1){
		...
		if (strncmp(ident_type, "Euler", 5) == 0){
			integrationMethod = (Euler<LIFTimeDrivenModel_1_3> *) new Euler<LIFTimeDrivenModel_1_3>(this);
		}
		else if (strncmp(ident_type, "RK2", 3) == 0){
			integrationMethod = (RK2<LIFTimeDrivenModel_1_3> *) new RK2<LIFTimeDrivenModel_1_3>(this);
		}
		else if (strncmp(ident_type, "RK4", 3) == 0){
			integrationMethod = (RK4<LIFTimeDrivenModel_1_3> *) new RK4<LIFTimeDrivenModel_1_3>(this);
		}
		else if (strncmp(ident_type, "BDF", 3) == 0 && atoi(&ident_type[3])>0 && atoi(&ident_type[3])<7){
			integrationMethod = (BDFn<LIFTimeDrivenModel_1_3> *) new BDFn<LIFTimeDrivenModel_1_3>(this, atoi(&ident_type[3]));
		}
		else if (strncmp(ident_type, "Bifixed_Euler", 13) == 0){
			integrationMethod = (Bifixed_Euler<LIFTimeDrivenModel_1_3> *) new Bifixed_Euler<LIFTimeDrivenModel_1_3>(this);
		}
		else if (strncmp(ident_type, "Bifixed_RK2", 11) == 0){
			integrationMethod = (Bifixed_RK2<LIFTimeDrivenModel_1_3> *) new Bifixed_RK2<LIFTimeDrivenModel_1_3>(this);
		}
		else if (strncmp(ident_type, "Bifixed_RK4", 11) == 0){
			integrationMethod = (Bifixed_RK4<LIFTimeDrivenModel_1_3> *) new Bifixed_RK4<LIFTimeDrivenModel_1_3>(this);
		}
		else if (strncmp(ident_type, "Bifixed_BDF", 11) == 0 && atoi(&ident_type[11]) == 2){
			integrationMethod = (Bifixed_BDFn<LIFTimeDrivenModel_1_3> *) new Bifixed_BDFn<LIFTimeDrivenModel_1_3>(this, atoi(&ident_type[11]));
		}
		else{
