/***************************************************************************
 *                           LIFTimeDrivenModel_1_2_NEW.cpp                *
 *                           -------------------                           *
...
#include "../../include/neuron_model/LIFTimeDrivenModel_1_2_NEW.h"
...
void LIFTimeDrivenModel_1_2_NEW::LoadNeuronModel(string ConfigFile) throw (EDLUTFileException){
	FILE *fh;
	long Currentline = 0L;
	...
	if(fh){
								...
								if(fscanf(fh,"%f",&this->tinh)==1 && this->tinh > 0.0f){
									...
									if (fscanf(fh, "%f", &this->tref) == 1 && this->tref >= 0.0f){
										...
										if(fscanf(fh,"%f",&this->grest)==1 && this->grest > 0.0f){
											...
											throw EDLUTFileException(TASK_LIF_TIME_DRIVEN_MODEL_1_2_NEW_LOAD, ERROR_LIF_TIME_DRIVEN_MODEL_1_2_NEW_GREST, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
										}
									} else {
										throw EDLUTFileException(TASK_LIF_TIME_DRIVEN_MODEL_1_2_NEW_LOAD, ERROR_LIF_TIME_DRIVEN_MODEL_1_2_NEW_TREF, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
									}
								} else {
									throw EDLUTFileException(TASK_LIF_TIME_DRIVEN_MODEL_1_2_NEW_LOAD, ERROR_LIF_TIME_DRIVEN_MODEL_1_2_NEW_TINH, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
								}
							} else {
								throw EDLUTFileException(TASK_LIF_TIME_DRIVEN_MODEL_1_2_NEW_LOAD, ERROR_LIF_TIME_DRIVEN_MODEL_1_2_NEW_TEXC, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
							}
						} else {
							throw EDLUTFileException(TASK_LIF_TIME_DRIVEN_MODEL_1_2_NEW_LOAD, ERROR_LIF_TIME_DRIVEN_MODEL_1_2_NEW_CM, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
						}
					} else {
						throw EDLUTFileException(TASK_LIF_TIME_DRIVEN_MODEL_1_2_NEW_LOAD, ERROR_LIF_TIME_DRIVEN_MODEL_1_2_NEW_VTHR, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
					}
				} else {
					throw EDLUTFileException(TASK_LIF_TIME_DRIVEN_MODEL_1_2_NEW_LOAD, ERROR_LIF_TIME_DRIVEN_MODEL_1_2_NEW_EREST, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
				}
			} else {
				throw EDLUTFileException(TASK_LIF_TIME_DRIVEN_MODEL_1_2_NEW_LOAD, ERROR_LIF_TIME_DRIVEN_MODEL_1_2_NEW_EINH, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
			}
		} else {
			throw EDLUTFileException(TASK_LIF_TIME_DRIVEN_MODEL_1_2_NEW_LOAD, ERROR_LIF_TIME_DRIVEN_MODEL_1_2_NEW_EEXC, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
		}
		...
		throw EDLUTFileException(TASK_LIF_TIME_DRIVEN_MODEL_1_2_NEW_LOAD, ERROR_NEURON_MODEL_OPEN, REPAIR_NEURON_MODEL_NAME, Currentline, ConfigFile.c_str(), true);
	}
	fclose(fh);
}

//this neuron model is implemented in a second scale.
LIFTimeDrivenModel_1_2_NEW::LIFTimeDrivenModel_1_2_NEW(string NeuronTypeID, string NeuronModelID) : TimeDrivenNeuronModel(NeuronTypeID, NeuronModelID, MilisecondScale), eexc(0), einh(0), erest(0), vthr(0), cm(0), texc(0), tinh(0),
		tref(0), grest(0){
}

LIFTimeDrivenModel_1_2_NEW::~LIFTimeDrivenModel_1_2_NEW(void)
{
}

void LIFTimeDrivenModel_1_2_NEW::LoadNeuronModel() throw (EDLUTFileException){
	this->LoadNeuronModel(this->GetModelID()+".cfg");
}

VectorNeuronState * LIFTimeDrivenModel_1_2_NEW::InitializeState(){
	return this->GetVectorNeuronState();
}


InternalSpike * LIFTimeDrivenModel_1_2_NEW::ProcessInputSpike(Interconnection * inter, double time){
...
bool LIFTimeDrivenModel_1_2_NEW::UpdateState(int index, double CurrentTime){
	//NeuronState[0] --> v 
	//NeuronState[1] --> gexc 
	//NeuronState[2] --> ginh 
...
enum NeuronModelOutputActivityType LIFTimeDrivenModel_1_2_NEW::GetModelOutputActivityType(){
	return OUTPUT_SPIKE;
}

enum NeuronModelInputActivityType LIFTimeDrivenModel_1_2_NEW::GetModelInputActivityType(){
	return INPUT_SPIKE;
}


ostream & LIFTimeDrivenModel_1_2_NEW::PrintInfo(ostream & out){
...
void LIFTimeDrivenModel_1_2_NEW::InitializeStates(int N_neurons, int OpenMPQueueIndex){
...
void LIFTimeDrivenModel_1_2_NEW::EvaluateSpikeCondition(float previous_V, float * NeuronState, int index, float elapsedTimeInNeuronModelScale){
...
void LIFTimeDrivenModel_1_2_NEW::EvaluateDifferentialEquation(float * NeuronState, float * AuxNeuronState, int index, float elapsed_time){
...
void LIFTimeDrivenModel_1_2_NEW::EvaluateTimeDependentEquation(float * NeuronState, int index, int elapsed_time_index){
	float limit=1e-9;
...
void LIFTimeDrivenModel_1_2_NEW::Calculate_conductance_exp_values(int index, float elapsed_time){
	//excitatory synapse.
...
bool LIFTimeDrivenModel_1_2_NEW::CheckSynapseType(Interconnection * connection){
...
void LIFTimeDrivenModel_1_2_NEW::loadIntegrationMethod(string fileName, FILE *fh, long * Currentline)throw (EDLUTFileException){
	...
	if (fscanf(fh, "%s", ident_type) == 1){
		...
		if (strncmp(ident_type, "Euler", 5) == 0){
			integrationMethod = (Euler<LIFTimeDrivenModel_1_2_NEW> *) new Euler<LIFTimeDrivenModel_1_2_NEW>(this);
		}
		else if (strncmp(ident_type, "RK2", 3) == 0){
			integrationMethod = (RK2<LIFTimeDrivenModel_1_2_NEW> *) new RK2<LIFTimeDrivenModel_1_2_NEW>(this);
		}
		else if (strncmp(ident_type, "RK4", 3) == 0){
			integrationMethod = (RK4<LIFTimeDrivenModel_1_2_NEW> *) new RK4<LIFTimeDrivenModel_1_2_NEW>(this);
		}
		else if (strncmp(ident_type, "BDF", 3) == 0 && atoi(&ident_type[3])>0 && atoi(&ident_type[3])<7){
			integrationMethod = (BDFn<LIFTimeDrivenModel_1_2_NEW> *) new BDFn<LIFTimeDrivenModel_1_2_NEW>(this, atoi(&ident_type[3]));
		}
		else if (strncmp(ident_type, "Bifixed_Euler", 13) == 0){
			integrationMethod = (Bifixed_Euler<LIFTimeDrivenModel_1_2_NEW> *) new Bifixed_Euler<LIFTimeDrivenModel_1_2_NEW>(this);
		}
		else if (strncmp(ident_type, "Bifixed_RK2", 11) == 0){
			integrationMethod = (Bifixed_RK2<LIFTimeDrivenModel_1_2_NEW> *) new Bifixed_RK2<LIFTimeDrivenModel_1_2_NEW>(this);
		}
		else if (strncmp(ident_type, "Bifixed_RK4", 11) == 0){
			integrationMethod = (Bifixed_RK4<LIFTimeDrivenModel_1_2_NEW> *) new Bifixed_RK4<LIFTimeDrivenModel_1_2_NEW>(this);
		}
		else if (strncmp(ident_type, "Bifixed_BDF", 11) == 0 && atoi(&ident_type[11]) == 2){
			integrationMethod = (Bifixed_BDFn<LIFTimeDrivenModel_1_2_NEW> *) new Bifixed_BDFn<LIFTimeDrivenModel_1_2_NEW>(this, atoi(&ident_type[11]));
		}
		else{
