...
#include "../../include/neuron_model/IzhikevichTimeDrivenModel_GPU.h"
#include "../../include/neuron_model/IzhikevichTimeDrivenModel_GPU2.h"
...
void IzhikevichTimeDrivenModel_GPU::LoadNeuronModel(string ConfigFile) throw (EDLUTFileException){
	FILE *fh;
	long Currentline = 0L;
	...
	if(fh){
								...
								if (fscanf(fh, "%f", &this->cm) == 1 && this->cm > 0.0f){
									...
									if (fscanf(fh, "%f", &this->texc) == 1 && this->texc > 0.0f){
										...
										if (fscanf(fh, "%f", &this->tinh) == 1 && this->tinh > 0.0f){
											...
											throw EDLUTFileException(TASK_IZHIKEVICH_TIME_DRIVEN_MODEL_GPU_LOAD, ERROR_IZHIKEVICH_TIME_DRIVEN_MODEL_TINH, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
										}
									}else {
										throw EDLUTFileException(TASK_IZHIKEVICH_TIME_DRIVEN_MODEL_GPU_LOAD, ERROR_IZHIKEVICH_TIME_DRIVEN_MODEL_TEXC, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
									}
								}else {
									throw EDLUTFileException(TASK_IZHIKEVICH_TIME_DRIVEN_MODEL_GPU_LOAD, ERROR_IZHIKEVICH_TIME_DRIVEN_MODEL_CM, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
								}
							}else {
								throw EDLUTFileException(TASK_IZHIKEVICH_TIME_DRIVEN_MODEL_GPU_LOAD, ERROR_IZHIKEVICH_TIME_DRIVEN_MODEL_EINH, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
							}
						}else {
							throw EDLUTFileException(TASK_IZHIKEVICH_TIME_DRIVEN_MODEL_GPU_LOAD, ERROR_IZHIKEVICH_TIME_DRIVEN_MODEL_EEXC, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
						}
					}else {
						throw EDLUTFileException(TASK_IZHIKEVICH_TIME_DRIVEN_MODEL_GPU_LOAD, ERROR_IZHIKEVICH_TIME_DRIVEN_MODEL_D, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
					}
				}else {
					throw EDLUTFileException(TASK_IZHIKEVICH_TIME_DRIVEN_MODEL_GPU_LOAD, ERROR_IZHIKEVICH_TIME_DRIVEN_MODEL_C, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
				}
			}else {
				throw EDLUTFileException(TASK_IZHIKEVICH_TIME_DRIVEN_MODEL_GPU_LOAD, ERROR_IZHIKEVICH_TIME_DRIVEN_MODEL_B, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
			}
		}else {
			throw EDLUTFileException(TASK_IZHIKEVICH_TIME_DRIVEN_MODEL_GPU_LOAD, ERROR_IZHIKEVICH_TIME_DRIVEN_MODEL_A, REPAIR_NEURON_MODEL_VALUES, Currentline, ConfigFile.c_str(), true);
		}
  		//INTEGRATION METHOD
		...
		throw EDLUTFileException(TASK_IZHIKEVICH_TIME_DRIVEN_MODEL_GPU_LOAD, ERROR_NEURON_MODEL_OPEN, REPAIR_NEURON_MODEL_NAME, Currentline, ConfigFile.c_str(), true);
	}
	fclose(fh);
}

IzhikevichTimeDrivenModel_GPU::IzhikevichTimeDrivenModel_GPU(string NeuronTypeID, string NeuronModelID): TimeDrivenNeuronModel_GPU(NeuronTypeID, NeuronModelID, MilisecondScale){
		//a=0.02f;
		//b=0.2f;
		//c=-65.0f;
		//d=8;
...
IzhikevichTimeDrivenModel_GPU::~IzhikevichTimeDrivenModel_GPU(void){
	DeleteClassGPU2();
}

void IzhikevichTimeDrivenModel_GPU::LoadNeuronModel() throw (EDLUTFileException){
	this->LoadNeuronModel(this->GetModelID()+".cfg");
}

VectorNeuronState * IzhikevichTimeDrivenModel_GPU::InitializeState(){
	return this->GetVectorNeuronState();
}


InternalSpike * IzhikevichTimeDrivenModel_GPU::ProcessInputSpike(Interconnection * inter, double time){
...
__global__ void IzhikevichTimeDrivenModel_GPU_UpdateState(IzhikevichTimeDrivenModel_GPU2 ** NeuronModel_GPU2, double CurrentTime){
	(*NeuronModel_GPU2)->UpdateState(CurrentTime);
}

		
bool IzhikevichTimeDrivenModel_GPU::UpdateState(int index, double CurrentTime){
	VectorNeuronState_GPU *state = (VectorNeuronState_GPU *) State;

	//----------------------------------------------
	if(prop.canMapHostMemory){
		IzhikevichTimeDrivenModel_GPU_UpdateState<<<N_block,N_thread>>>(NeuronModel_GPU2, CurrentTime);
	}else{
...
enum NeuronModelOutputActivityType IzhikevichTimeDrivenModel_GPU::GetModelOutputActivityType(){
	return OUTPUT_SPIKE;
}

enum NeuronModelInputActivityType IzhikevichTimeDrivenModel_GPU::GetModelInputActivityType(){
	return INPUT_SPIKE;
}


ostream & IzhikevichTimeDrivenModel_GPU::PrintInfo(ostream & out){
	return out;
}	


void IzhikevichTimeDrivenModel_GPU::InitializeStates(int N_neurons, int OpenMPQueueIndex){
...
__global__ void IzhikevichTimeDrivenModel_GPU_InitializeClassGPU2(IzhikevichTimeDrivenModel_GPU2 ** NeuronModel_GPU2, 
		float a, float b, float c, float d, float eexc, float einh, float cm, float texc, float tinh, 
		char const* integrationName, int N_neurons, void ** Buffer_GPU){
	if(blockIdx.x==0 && threadIdx.x==0){
		(*NeuronModel_GPU2)=new IzhikevichTimeDrivenModel_GPU2(a, b, c, d, eexc, einh, cm, texc, tinh, integrationName, 
			N_neurons, Buffer_GPU);
	}
}

void IzhikevichTimeDrivenModel_GPU::InitializeClassGPU2(int N_neurons){
	cudaMalloc(&NeuronModel_GPU2, sizeof(IzhikevichTimeDrivenModel_GPU **));
	...
	IzhikevichTimeDrivenModel_GPU_InitializeClassGPU2<<<1,1>>>(NeuronModel_GPU2, a, b, c, d, eexc, einh,  
		cm, texc, tinh, integrationNameGPU, N_neurons, integrationMethod_GPU->Buffer_GPU);
...
void IzhikevichTimeDrivenModel_GPU::InitializeVectorNeuronState_GPU2(){
	VectorNeuronState_GPU *state = (VectorNeuronState_GPU *) State;
...
void IzhikevichTimeDrivenModel_GPU::DeleteClassGPU2(){
    DeleteClass_GPU2<<<1,1>>>(NeuronModel_GPU2);
    cudaFree(NeuronModel_GPU2);
}


bool IzhikevichTimeDrivenModel_GPU::CheckSynapseType(Interconnection * connection){
