/***************************************************************************
 *              ROSSpikeDecoderCompactFutureTorque_compactDelay.cpp        *
 *                           -------------------                           *
...
void ROSSpikeDecoderCompactFutureTorque_compactDelay::SpikeCallback(const edlut_ros::Spike_group::ConstPtr& msg){
...
ROSSpikeDecoderCompactFutureTorque_compactDelay::ROSSpikeDecoderCompactFutureTorque_compactDelay(std::string input_topic_name,
				...
				last_spike_time(0.0)
{
	...
	ros::SubscribeOptions subscriberOptions = ros::SubscribeOptions::create<edlut_ros::Spike_group>(
	 		input_topic_name, 1000, boost::bind(&ROSSpikeDecoderCompactFutureTorque_compactDelay::SpikeCallback, this, _1), ros::VoidPtr(), &this->CallbackQueue);
...
ROSSpikeDecoderCompactFutureTorque_compactDelay::~ROSSpikeDecoderCompactFutureTorque_compactDelay() {
	// TODO Auto-generated destructor stub
}

std::vector<edlut_ros::AnalogCompactDelay> ROSSpikeDecoderCompactFutureTorque_compactDelay::UpdateDecoder(double end_time){
	...
	if (!this->activity_queue.empty()){
		...
		while (!(this->activity_queue.empty()) &&	top_spike.time<=checkpoint){
			...
			if (top_spike.time > checkpoint){
				edlut_ros::AnalogCompactDelay newTorque;
				...
				double elapsed_time = this->time_step;
