...
#include "edlut_ros/TrafficDelay.h"
...
int main(int argc, char **argv)
{
	...
	std::string network_file, weight_file, input_topic, output_topic, clock_topic, input_delay_topic;
	...
	ros::Subscriber clock_subscriber, delay_subscriber;
	ExternalClock ext_clock;
	TrafficDelay msgDelay;
	double motor_delay = 0.0;
	double save_weight_period = 0.0;
	double sensorial_delay_margin;
	...
	private_node_handle_.getParam("sensorial_delay_margin", sensorial_delay_margin);
	private_node_handle_.getParam("motor_delay", motor_delay);
	...
	private_node_handle_.getParam("input_delay_topic", input_delay_topic);
	...
	delay_subscriber = nh.subscribe(input_delay_topic, 1000, &TrafficDelay::InputCallback, &msgDelay);
	...
	try{
		...
		// Simul.AddInputSpikeDriver(new ROSTopicInputDriver(input_topic, max_spike_buffered, InitSimulationRosTime.toSec(), &nh, &CallbackQueue));
...
**/
		...
		// Simul.RealTimeRestrictionObject->SetParameterWatchDog(simulation_step_time, (sensorial_delay+motor_delay)*0.9, rt1_gap, rt2_gap, rt3_gap);
		Simul.RealTimeRestrictionObject->SetParameterWatchDog(simulation_step_time, sensorial_delay*sensorial_delay_margin, rt1_gap, rt2_gap, rt3_gap);
		...
		if(omp_get_thread_num()==1){
			...
			while (!stop_simulation){
				...
				if(!use_sim_time){
					...
					// Simul.RealTimeRestrictionObject->NextStepWatchDog(ros::Time::now().toSec() - InitSimulationRosTime.toSec() - motor_delay*0.9);
					Simul.RealTimeRestrictionObject->NextStepWatchDog(ros::Time::now().toSec() - InitSimulationRosTime.toSec());


					// (sensorial_delay - wifi_Path_delay_of_each_msg)
					Simul.RealTimeRestrictionObject->SetMaxSimulationTimeInAdvance(sensorial_delay*sensorial_delay_margin - msgDelay.GetLastDelay());
					if ( (sensorial_delay*sensorial_delay_margin - msgDelay.GetLastDelay()) < 0.002){
						ROS_INFO("WARNING: Sensorial Delay = %f . Current Delay = %f ", sensorial_delay*sensorial_delay_margin, msgDelay.GetLastDelay());
					}
				}
