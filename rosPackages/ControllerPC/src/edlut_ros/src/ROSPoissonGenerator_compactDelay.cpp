/***************************************************************************
 *                           ROSPoissonGenerator_compactDelay.cpp                       *
 *                           -------------------                           *
...
ROSPoissonGenerator_compactDelay::ROSPoissonGenerator_compactDelay(std::vector<unsigned int> num_filters_row, //
...
ROSPoissonGenerator_compactDelay::~ROSPoissonGenerator_compactDelay() {
	// TODO Auto-generated destructor stub
}


std::vector<std::vector<double> > ROSPoissonGenerator_compactDelay::GetCenters(){
	return this->centers;
}
 ...
 */
void ROSPoissonGenerator_compactDelay::GenerateActivity(double input_signal_value, double current_time, int joint, double new_sampling_period, double delayMsg){
	...
	for (unsigned int index=0, id_neuron=this->init_neuron[joint]; id_neuron<=this->end_neuron[joint]; ++index, ++id_neuron){
		...
		if (rand()/double(RAND_MAX) < sp_firing_rate*new_sampling_period){
			...
			for (int i=0; i< n_spikes; i++){
					// this->time_array.push_back(current_time + i * 0.002 - delayMsg);

					// The delay of each message does not have to be substracted explicitly.
					// The delay is taken into account since the current_time is the generation time of the message
