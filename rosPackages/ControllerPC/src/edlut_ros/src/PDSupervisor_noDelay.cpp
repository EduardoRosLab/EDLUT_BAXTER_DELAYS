...
#include "edlut_ros/PDSupervisor_noDelay.h"
...
void PDSupervisor_noDelay::CurrentPositionCallback(const edlut_ros::AnalogCompact::ConstPtr& msg){
...
void PDSupervisor_noDelay::CurrentVelocityCallback(const edlut_ros::AnalogCompact::ConstPtr& msg){
	// Check if the message is newer than previous ones.
...
PDSupervisor_noDelay::PDSupervisor_noDelay(std::string current_position_topic,
	...
	this->current_position_subs = this->NodeHandler.subscribe(current_position_topic, 1e9, &PDSupervisor_noDelay::CurrentPositionCallback, this);
	ROS_DEBUG("PD Supervisor: Subscribed to topic %s for current positions", current_position_topic.c_str());

	this->current_velocity_subs = this->NodeHandler.subscribe(current_velocity_topic, 1e9, &PDSupervisor_noDelay::CurrentVelocityCallback, this);
...
PDSupervisor_noDelay::~PDSupervisor_noDelay() {
 ...
 */
void PDSupervisor_noDelay::NextSupervisorStep(){
