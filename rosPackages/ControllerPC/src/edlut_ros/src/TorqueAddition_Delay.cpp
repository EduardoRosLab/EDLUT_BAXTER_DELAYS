...
void TorqueAddition_Delay::TorqueCerebellumCallback(const edlut_ros::AnalogCompactDelay::ConstPtr& msg){
...
void TorqueAddition_Delay::InserAndOrderCerebellarBufferFuture(edlut_ros::AnalogCompact & newMessage, std::vector<edlut_ros::AnalogCompact>  & buffer){
...
// Order the torque buffer. The last element is the oldest motor command.
void TorqueAddition_Delay::InserAndOrderCerebellarBufferPast(edlut_ros::AnalogCompact & newMessage, std::vector<edlut_ros::AnalogCompact>  & buffer){
  //insert new message in first position
	buffer.insert(buffer.begin(), newMessage);
...
void TorqueAddition_Delay::TorqueSupervisorCallback(const edlut_ros::AnalogCompact::ConstPtr& msg){
	if (!this->first_msg){
		this->first_msg = true;
		this->init_time_stamp = msg->header.stamp.toSec();
	}
...
int TorqueAddition_Delay::FindJointIndex(std::vector<std::string> strvector, std::string name){
...
TorqueAddition_Delay::TorqueAddition_Delay(
...
ros::SubscribeOptions subscriberOptions = ros::SubscribeOptions::create<edlut_ros::AnalogCompactDelay>(
	torque_cerebellum_topic_name, 10.0, boost::bind(&TorqueAddition_Delay::TorqueCerebellumCallback, this, _1), ros::VoidPtr(), this->CallbackQueue);
...
this->subscriber_torque_cerebellum = this->NodeHandler->subscribe(subscriberOptions);
	...
	//this->subscriber_torque_cerebellum = this->NodeHandler->subscribe(torque_cerebellum_topic_name, 10.0, &TorqueAddition_Delay::TorqueCerebellumCallback, this);
	this->subscriber_torque_supervisor = this->NodeHandler->subscribe(torque_supervisor_topic_name, 10.0, &TorqueAddition_Delay::TorqueSupervisorCallback, this);
...
void TorqueAddition_Delay::UpdateTorque(ros::Time current_time){
...
void TorqueAddition_Delay::CleanCerebellarBuffer(double end_time){
