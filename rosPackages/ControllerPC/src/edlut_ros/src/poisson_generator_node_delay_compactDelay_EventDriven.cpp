...
#include <edlut_ros/ROSPoissonGenerator_compactDelay.h>
...
public:
	...
	void AnalogCallback(const edlut_ros::AnalogCompactDelay::ConstPtr& msg){
		...
		this->delayMsg = msg->delay;

		this->last_time_stamp = msg->header.stamp.toSec();
...
int main(int argc, char **argv)
{
  ...
  nh.getParam("use_sim_time", use_sim_time);
	...
	ROSPoissonGenerator_compactDelay objPoissonGeneratorPos(
			num_filters_pos,
			min_value,
			max_value,
	...
	ROSPoissonGenerator_compactDelay objPoissonGeneratorNeg(
			num_filters_neg,
			min_value,
			max_value,
	...
	while (!stop_node){
		...
		if (use_sim_time){
				...
				if (objSensorState.IsInLearning()){
					...
					for (unsigned int i= 0; i<joint_list.size(); ++i){
						...
						if (objSensorState.GetSensorValue(i) > 0){
							objPoissonGeneratorPos.GenerateActivity(objSensorState.GetSensorValue(i), time.toSec(), i, new_sampling_period.toSec(), objSensorState.GetDelayMsg());
						}
						else{
							objPoissonGeneratorNeg.GenerateActivity(-objSensorState.GetSensorValue(i), time.toSec(), i, new_sampling_period.toSec(), objSensorState.GetDelayMsg());
						}
					}
			...
			// time = ros::Time::now();
			// Generate activity when a new analogue value is received
			if (objSensorState.IsInLearning() && objSensorState.NewValue() ){
				ROS_DEBUG("Poisson Generator node: Generating activity at time %f", objSensorState.GetLastTimeStamp());
				edlut_ros::Spike_group spike_group;
				for (unsigned int i=0; i<joint_list.size(); ++i){
					if (objSensorState.GetSensorValue(i) > 0){
						objPoissonGeneratorPos.GenerateActivity(objSensorState.GetSensorValue(i), objSensorState.GetLastTimeStamp(), i, sampling_period, objSensorState.GetDelayMsg());
					}
					else{
						objPoissonGeneratorNeg.GenerateActivity(-objSensorState.GetSensorValue(i), objSensorState.GetLastTimeStamp(), i, sampling_period, objSensorState.GetDelayMsg());
					}
				}
				// Poisitive error spikes
				for (unsigned int j=0; j<objPoissonGeneratorPos.neuron_index_array.size(); j++){
			...
			objSensorState.WaitForNewValue();
			// rate.sleep();
		}
	}
