...
public:
	RTT (std::string output_topic):
		output_topic(output_topic){
		ros::NodeHandle nh;

		this->publisher = nh.advertise<edlut_ros::Analog>(output_topic, 10);
	...
	void EventCallback(const edlut_ros::AnalogCompact::ConstPtr& msg){
		...
		new_msg.data = ros::Time::now().toSec() - msg->data[0];
...
{
	// Set up ROS.
	ros::init(argc, argv, "rtt_compute", ros::init_options::NoSigintHandler);
	...
	RTTOptions.transport_hints = ros::TransportHints().reliable().tcpNoDelay(true);
