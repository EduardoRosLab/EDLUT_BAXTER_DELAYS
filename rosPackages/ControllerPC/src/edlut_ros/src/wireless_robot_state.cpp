...
public:
	...
	void EventCallback(const sensor_msgs::JointState::ConstPtr& msg){
		...
		if (!this->use_sim_time){
			...
			double time_stamp = round(this->sampling_frequency * ros::Time::now().toSec()) * this->time_step;
			if (round(this->sampling_frequency *(time_stamp - this->last_time_stamp) ) * this->time_step < this->time_step){
				this->discard_sample = true;
			}
			else{
				this->discard_sample = false;
				this->last_time_stamp = time_stamp;
				CurrentTime = ros::Time(time_stamp);
			}
		}
		else{
