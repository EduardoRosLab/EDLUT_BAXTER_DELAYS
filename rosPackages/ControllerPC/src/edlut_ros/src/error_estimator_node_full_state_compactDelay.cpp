...
{
	// Set up ROS.
	ros::init(argc, argv, "error_estimator_full_state_node");
	...
	std::string full_state_topic,  output_topic;
	...
	private_node_handle_.getParam("full_state_topic", full_state_topic);
	...
	if (use_sim_time){
		ROS_DEBUG("Error estimator full_state node: Subscribing to topic /clock_sync");
	...
	ROSErrorCalculator_FullStateCompactDelay objErrorCalculator(full_state_topic,
			output_topic,
			joint_list,
			error_pos_gain,
			error_vel_gain);
