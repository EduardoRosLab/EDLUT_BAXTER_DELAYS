...
ROSRBFBank_compactDelay::ROSRBFBank_compactDelay(std::vector<unsigned int> num_filters_row,
...
ROSRBFBank_compactDelay::~ROSRBFBank_compactDelay() {
	// TODO Auto-generated destructor stub
}

std::vector<std::vector<double> > ROSRBFBank_compactDelay::GetRBFCenters(){
	return this->centers;
}

void ROSRBFBank_compactDelay::SetPublisher(ros::Publisher * newPublisher){
	this->Publisher = newPublisher;
}

void ROSRBFBank_compactDelay::gaussian_function(double x, std::vector<double> centers, std::vector<double> witdhs, std::vector<double> &output_values)
{
	 // SQUARE
...
void ROSRBFBank_compactDelay::calculate_gaussian_amplitude(){
	this->amplitude = 1.0;
	return;
}
 ...
 */
void ROSRBFBank_compactDelay::GenerateActivity(std::vector<double> current_values, double cur_simulation_time, double new_sampling_period, double delayMsg){
		...
		for (int i=0; i< this->num_filters_row.size(); i++){
			...
			for (int j=0; j< this->num_filters_row[i]; j++){
				...
				for(;spike_time[j]<cur_simulation_time+new_sampling_period;spike_time[j]+=spike_period[j]){
					...
					// this->time_array.push_back(spike_time[j] + delay - delayMsg);
