...
public:
  ...
  void CircleEightSequence(){
    double dt = 2*M_PI / (this->samples/2.0);
    for (int n=0; n<this->num_circles; n++){
      for (int i=0; i<this->samples/2; i++){
        this->x[i+(n*this->samples)] = this->radius * sin(M_PI + (i*dt));
        this->y[i+(n*this->samples)] = this->radius * cos(M_PI + (i*dt));
        this->z[i+(n*this->samples)] = this->start_z;
      }
      for (int i=this->samples/2; i<this->samples; i++){
        this->x[i+(n*this->samples)] = this->radius * sin(2* (M_PI - (i*dt)))/2.0;
        this->y[i+(n*this->samples)] = this->radius * cos(M_PI + (i*dt));
        this->z[i+(n*this->samples)] = this->start_z;
      }
    }
...
int main(int argc, char** argv)
{
  ...
  if (mode == "horizontal" or mode =="eight" or mode == "lissajou" or mode == "inclined" or mode == "sequence"){
  ...
  if (mode == "horizontal" or mode == "lissajou" or mode == "inclined" or mode == "eight" or mode == "sequence"){
  ...
  file_xyz.open("/home/baxter/catkin_ws_WiFi/src/BaxterCerebellum/src/edlut_ros/trajectory_circle_eight_sequence/moveit_circle_eight_sequence_xyz.txt");

  if (mode=="horizontal" or mode == "eight" or mode == "lissajou" or mode == "sequence"){
  ...
  //
  std::ofstream file;
  file.open("/home/baxter/catkin_ws_WiFi/src/BaxterCerebellum/src/edlut_ros/trajectory_circle_eight_sequence/moveit_circle_eight_sequence_position.txt");
  for (int i=0; i<trajectory.joint_trajectory.points.size(); i++){
