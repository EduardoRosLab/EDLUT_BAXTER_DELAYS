 ...
 ***************************************************************************/
 ...
 #include <edlut_ros/ROSRBFBank_compactDelay.h>
 ...
 public:
    ...
    // Register the transmission delay of the current message
 ...
 int main(int argc, char **argv)
 {
  ...
  ros::Publisher output_publisher_group = nh.advertise<edlut_ros::Spike_group>(output_topic, 1e9);
 	...
 	std::vector<ROSRBFBank_compactDelay> objRBFBankVector;
  ...
  ROSRBFBank_compactDelay objRBFBank(
      num_filters,
      &output_publisher,
      min_values,
      max_values,
 	...
 	while (!stop_node){
        ...
        ros::Duration new_sampling_period = new_time - time;
          ...
          edlut_ros::Spike_group spike_group;
					objRBFBank.GenerateActivity(objSensorState.GetSensorValue(), objSensorState.GetLastTimeStamp(), new_sampling_period.toSec(), objSensorState.GetDelayMsg());
      ...
      // Generate activity when a new analogue value is received
			if (objSensorState.IsInLearning() && objSensorState.NewValue()){
				ROS_DEBUG("RBF node: Generating activity at time %f", objSensorState.GetLastTimeStamp());
        edlut_ros::Spike_group spike_group;
        objRBFBank.GenerateActivity(objSensorState.GetSensorValue(), objSensorState.GetLastTimeStamp(), sampling_period, objSensorState.GetDelayMsg());
        spike_group.neuron_index = objRBFBank.neuron_index_array;
        spike_group.time = objRBFBank.time_array;
        objRBFBank.neuron_index_array.clear();
        objRBFBank.time_array.clear();
