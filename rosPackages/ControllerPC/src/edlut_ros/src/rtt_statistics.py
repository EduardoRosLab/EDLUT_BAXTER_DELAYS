...
class Rtt_statistics(object):
	...
	def RttCallback(self, data):
		self.rtt_data.append(data.data)
	...
	def GetCDFRtt(self):
		# sort the data:
		self.rtt_data_sorted = numpy.sort(self.rtt_data)

		# calculate the proportional values of samples
		self.rtt_p = 1. * numpy.arange(len(self.rtt_data)) / (len(self.rtt_data) - 1)
	...
	def CDFStatistics(self):
		...
		print "________________________________________________"
		print "	5%  --> " , self.rtt_data_sorted[(len(self.rtt_data_sorted) - 1) * 5 / 100]
		print "	10% --> " , self.rtt_data_sorted[(len(self.rtt_data_sorted) - 1) * 10 / 100]
		print "	20% --> " , self.rtt_data_sorted[(len(self.rtt_data_sorted) - 1) * 20 / 100]
		print "	40% --> " , self.rtt_data_sorted[(len(self.rtt_data_sorted) - 1) * 40 / 100]
		print "	50% --> " , self.rtt_data_sorted[(len(self.rtt_data_sorted) - 1) * 50 / 100]
		print "	70% --> " , self.rtt_data_sorted[(len(self.rtt_data_sorted) - 1) * 70 / 100]
		print "	80% --> " , self.rtt_data_sorted[(len(self.rtt_data_sorted) - 1) * 80 / 100]
		print "	90% --> " , self.rtt_data_sorted[(len(self.rtt_data_sorted) - 1) * 90 / 100]
		print "	95% --> " , self.rtt_data_sorted[(len(self.rtt_data_sorted) - 1) * 95 / 100]
		print "	99% --> " , self.rtt_data_sorted[(len(self.rtt_data_sorted) - 1) * 99 / 100]
		print "	100% --> " , self.rtt_data_sorted[len(self.rtt_data_sorted) - 1]
	...
	def __init__(self):
		...
		#Subscribing to the topic
		self.rtt_sub = rospy.Subscriber(self.rtt_topic, edlut_ros.msg.Analog, self.RttCallback, queue_size=None)
	...
	def plot(self):
		line0 = plt.plot(self.rtt_data_sorted, self.p, color='b')
