...
class Synchronizer {
private:
	ros::Publisher output_Publisher, gravity_Publisher, delay_Publisher;
...
public:
		...
		disable_gravity_compensation(disable_gravity_compensation)
	{
		...
		this->delay_Publisher = nh.advertise<edlut_ros::Time>(delay_topic, 1);
	...
	void TimeDifferenceCallback(const edlut_ros::Time::ConstPtr& msg){
		this->time_difference = msg->data;
		return;
	}
	...
	void EventCallback(const edlut_ros::AnalogCompact::ConstPtr& trq){
		...
		this->delay = this->time_difference + ros::Time::now().toSec() - trq->header.stamp.toSec();
		delayMsg.data = this->delay;
		delayMsg.header.stamp = ros::Time::now();
		this->delay_Publisher.publish(delayMsg);
...
int main(int argc, char **argv)
{
	...
	std::vector<std::string> joint_list;
	std::string control_topic, gravity_topic, time_difference_topic, delay_topic;
	bool disable_gravity_compensation;
	...
	private_node_handle_.getParam("time_difference_topic", time_difference_topic);
	private_node_handle_.getParam("delay_topic", delay_topic);

	Synchronizer objSynchronizer =Synchronizer(output_topic, limb, joint_list, disable_gravity_compensation, gravity_topic, delay_topic);
...
//ros::Subscriber input_subscriber= nh.subscribe(baxterStateOptions);
	...
	ros::Subscriber time_difference_subscriber = nh.subscribe(time_difference_topic, 1, &Synchronizer::TimeDifferenceCallback, &objSynchronizer);
