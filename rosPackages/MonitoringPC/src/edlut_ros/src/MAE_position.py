...
class MaeplotAnimation(matplotlib.animation.TimedAnimation):
	...
	# Callback for the fullState messages
	def FullStateCallback(self, data):
		# Get index of the joints to be computed
		if self.first_msg:
			for joint, value in self.current_values.items():
				for index, name in enumerate(data.names):
					if data.names[index] == joint:
						self.aux_index.append(index)
			self.first_msg = False

		# Start counting time to control the duration of the trial
		if self.init_error:
			self.start = rospy.get_time()
			self.init_error = False
		now = rospy.get_time()

		# If time is within a trial, accumulate the error
		if now - self.start < self.duration_trial:
			counter = 0
			for joint, value in self.current_values.items():
				self.current_values[joint] = data.current_position[self.aux_index[counter]]
				self.desired_values[joint] = data.desired_position[self.aux_index[counter]]
				counter += 1
		...
		# If the trial is over, compute the trial's MAE
		else:
	...
	def new_figure(self):
		...
		self.line = matplotlib.pyplot.Line2D([], [], color='red', marker='.', markersize=3,linestyle='solid', label = self.full_stateTopic)
		self.mean_line = matplotlib.pyplot.Line2D([], [], color='blue', marker='.', markersize=3, linestyle=':', label = self.full_stateTopic)
	...
	def __init__(self):
		...
		self.full_stateTopic = rospy.get_param("~full_state_topic")
		...
		rospy.Subscriber(self.full_stateTopic, edlut_ros.msg.FullStateCompactDelay, self.FullStateCallback, queue_size=None)
